...
Rust has _memory-safety_ to avoid a whole class of bugs
...
compile-time; there is no run-time making checks. The one
exception here is array bound checks that are done by the compiled code at
run-time, be that the Rust compiler. It
is [possible to write unsafe code in Rust][unsafe-rust], and in fact,
...
In JavaScript, there is no concept of ownership of memory beyond the GC roots
...
types and writing code, a JavaScript developer can remain oblivious to ownership,
...
In JavaScript, references are shared freely without much thought so the idea
...
The `new` keyword in JavaScript creates an instance of a "type", and while members such
